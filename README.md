...
bash```docker-compose up --build - запуск 
docker-compose down - завершение```

### Через Makefile

bash```make up_compose - запуск
...
Может назначать роли и разрешения другим пользователям. При регистрации нового пользователя ему НЕ выдается роль автоматически.
